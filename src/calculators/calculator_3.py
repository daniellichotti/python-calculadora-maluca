...
from src.errors.http_bad_request import BadRequestError
...
class Calculator_3:
  ...
  def calculate(self, request: FlaskRequest) -> Dict: #type: ignore
    ...
    variance = self.__calculate_variances(input_data)
  ...
  def __calculate_variances(self, numbers: List[float]) -> float:
  ...
  def __verify_results(self, variance: float, multiplication: float) -> None:
    if variance < multiplication: 
      raise BadRequestError('Falha no processo: Variância menor que multiplicação.')
